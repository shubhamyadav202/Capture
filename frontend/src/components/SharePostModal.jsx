...
const SharePostModal = ({ postId, onClose }) => {
  ...
  const handleShare = async (receiverId) => {
    ...
    try {
      await axios.post(
        `${serverUrl}/api/message/share/${receiverId}`,
        { postId },
  ...
  return (
    ...
    >
      <div
        className="w-[90%] max-w-[420px] bg-white rounded-2xl shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-[20px] py-[15px] border-b border-gray-200">
          <h2 className="text-[18px] font-bold text-gray-900">Share Post</h2>
          <button
            onClick={onClose}
            className="p-1 rounded-full hover:bg-gray-100 transition-colors cursor-pointer"
          >
            <IoClose className="w-[24px] h-[24px] text-gray-600" />
          </button>
        </div>

        {/* Search */}
        <div className="px-[20px] py-[12px] border-b border-gray-100">
          <input
            type="text"
            placeholder="Search users..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full px-[14px] py-[10px] bg-gray-100 rounded-xl text-[14px] outline-none focus:ring-2 focus:ring-purple-400 transition-all"
          />
        </div>
        ...
        <div className="max-h-[350px] overflow-auto">
          {filteredUsers.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-[40px] text-gray-400">
            ...
            filteredUsers.map((user) => {
              ...
              return (
                <div
                  key={userId}
                  className="flex items-center justify-between px-[20px] py-[12px] hover:bg-gray-50 transition-colors"
                >
                  <div className="flex items-center gap-[12px] flex-1 min-w-0">
                    <div className="w-[44px] h-[44px] rounded-full overflow-hidden border-2 border-gray-200 flex-shrink-0">
                      <img
